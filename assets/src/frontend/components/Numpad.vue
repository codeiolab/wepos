<script>
	...
	export default {
				...
				return this.lines.map(line => {
					...
					stream.forEach(char => {
						if (char === '{') {
							...
							if (['append', 'backspace', 'space', 'clear', 'goto', 'doubleZero', 'cancel', 'pay'].indexOf(action) >= 0) method = this[action].bind(this, args);
		...
		methods: {
			...
			/**
			 * Push 00 when double zero button is clicked
			 */
			doubleZero() {
				this.append('00');
			},
			...
			/**
			 * Go back to sale screen when Cancel is clicked
			 */
			cancel() {
				this.$emit('back-to-sale');
			},
...
<template>
	<aside class="vue-keyboard" role="application" :data-value="value" :data-layout="layout">
